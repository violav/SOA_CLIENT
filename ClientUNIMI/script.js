...
var url = "https://apisoastudentunimi.herokuapp.com/api/v1/"; //"https://testviola2.herokuapp.com/api/v1/"; //  "http://localhost:1337/api/v1/"; // 
var userIdToModify;
...
scotchApp.controller('usersController', function ($scope, $rootScope, $http, $location) {
    ...
    //$scope.addStudent = function (name, password, email) {
    ...
    //    }).success(function (response) {
    //        alert('allowed');
    //        writeLog($rootScope.user, 'added user : ' + $scope.user, $rootScope.token)
    //        $scope.msg = "room added"
    //        // $scope.Scheduling = response;
    //        f1();
    //    }).error(function (response) {
    ...
    $scope.deleteUser = function (id) {
        ...
        $http({
            ...
            alert('allowed');
            writeLog($rootScope.user, 'deleted user : ' + id, $rootScope.token);
    ...
    $scope.UserDetail = function (utente) {
        userIdToModify = utente._id;
        $location.path('/userDetail');
    }
...
scotchApp.controller('userDetailController', ['$rootScope', '$scope', 'Auth', '$http', '$location', function ($rootScope, $scope, Auth, $http, $location) {
    //submit
    $scope.email = null;
    $scope.password = null;
    $scope.message = null;
    ...
    $scope.createNew = function () {
        ...
        $http({
            method: 'POST',
            url: u + 'authentication/register', // 'users',
            data: {
                email: $scope.student.email,
                password: $scope.student.password,
                name: $scope.student.name,
...
scotchApp.controller('schedulingCtrl', function ($scope, $rootScope, $http) {
    ...
    $scope.deleteScheduling = function (toDelete) {
        ...
        $http({
            ...
            $scope.msg = "Your booking has successfully deleted. Please refresh the page"
        }).error(function (response) {
            $scope.isError = true;
    ...
    $scope.addSchedule = function (idRoom, dataDa, dataA) {
        ...
        if (check) {
            $http({
                ...
                alert("unable to schedule the room " + response);
            });
        } else {
